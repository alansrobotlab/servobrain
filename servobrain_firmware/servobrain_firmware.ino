...
void updatePos() {
  ...
  //ADMUX = (1 << REFS1) |  | SENSORPIN;        // ADC0 (PB4) and Ref voltage = 1.1V 
  //ADMUX =  (1 << REFS2) | (1 << REFS1) | SENSORPIN;         // ADC0 (PB4) and Ref voltage = 2.56V without external bypass capacitor;
  ADMUX = SENSORPIN;          // ADC0 (PB4) and Ref voltage = VCC 
  ...
  if (sampleCount >= MAX_SAMPLES) {
    ...
    reg[POSITION] = mapValue(sampleBucket, reg[MINSENSOR], reg[MAXSENSOR], reg[MINGOAL], reg[MAXGOAL]);
    sampleBucket = 0.0f;
    sampleCount = 0;
  }
...
// From Arduino WMath.cpp
long mapValue(long x, long in_min, long in_max, long out_min, long out_max)
{
  return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}

// From Arduino wiring.h
...
SIGNAL(TIMER0_COMPA_vect) {
  ...
  if (counter >= INTERVAL) {
    ...
    */
    ...
    reg[GOAL] = constrainValue(reg[GOAL], reg[MINGOAL], reg[MAXGOAL]);
    reg[PULSE] = mapValue(reg[GOAL], reg[MINGOAL], reg[MAXGOAL], reg[MINPULSE], reg[MAXPULSE]);
  ...
  */
  ...
  digitalWrite(LEDPIN, heartbeats[((millis() >> 7) & 7)]);
